...
export function NotesSection({ handle, initialNotes, currentUserId, isAdmin }: Props) {
  ...
  const canModify = (n: NoteDto) => Boolean(isAdmin) || n.authorApiUserId === currentUserId;

  async function add() {
    if (!draft.trim()) return;
    setBusy(true);
    const res = await createNoteAction(handle, draft);
    setBusy(false);
    if (res.ok && res.note) {
  ...
  async function saveEdit(id: number) {
    setBusy(true);
    const res = await updateNoteAction(id, editDraft);
    setBusy(false);
    if (res.ok && res.note) {
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          <textarea
            ...
            placeholder="Ajouter une note sur ce citoyen…"
            rows={3}
            maxLength={10000}
